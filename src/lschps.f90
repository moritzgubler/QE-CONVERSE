...
!
! Vendored into QE-CONVERSE from atomic/src/lschps.f90 so that the
! Fermi-contact core-relaxation routine (core_relax.f90) can recompute
! all-electron s orbitals without linking the whole atomic (ld1.x) code.
! Only the non-relativistic bound-state solver (mode=2) is needed here.
! The single dependence on ld1inc (cau_fact, used only in the
! scalar-relativistic branches) is replaced by a local parameter.
!
SUBROUTINE lschps (mode, z, eps, grid, nin, n, l, e, v, u, nstop)
  ...
  IMPLICIT NONE
  !
  ! cau_fact = speed of light in atomic units (used only in the
  !            scalar-relativistic branches; core_relax calls mode=2)
  real(DP), PARAMETER :: cau_fact = 137.03599911_dp
  !
  ! I/O variables
  !
