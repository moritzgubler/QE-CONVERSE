...
In the implementation, the full double sum over $(\mu,\nu)$ is never
performed. For a given component $\gamma$ one selects the unique cyclic
pair $(\mu,\nu)\equiv(ii,jj)$ with $\epsilon_{\gamma\,ii\,jj}=+1$ and
evaluates the two terms that contribute:
\begin{equation}
\frac{i\alpha}{2}\bigl[T_{ii,jj} - T_{jj,ii}\bigr],
\end{equation}
where
\begin{equation}
T_{\mu\nu}
...
| u_{n\mathbf{k}} \rangle .
\end{equation}
Because $V_{mm'}^I$ is Hermitian, swapping $m\leftrightarrow m'$ gives
$T_{jj,ii}=T_{ii,jj}^{*}$, so
\begin{equation}
\frac{i\alpha}{2}\bigl[T_{ii,jj} - T_{ii,jj}^{*}\bigr]
=
\frac{i\alpha}{2}\cdot 2i\,\mathrm{Im}(T_{ii,jj})
=
-\alpha\,\mathrm{Im}(T_{ii,jj}).
\end{equation}
The sign flip comes from $i^2=-1$; no Levi-Civita contraction is needed.
Writing $\mathrm{tmp}=T_{ii,jj}$, this is the code line
\begin{verbatim}
delta_M_hub(kk) = delta_M_hub(kk) - 2.d0*imag(tmp)
\end{verbatim}
(the overall $\alpha/2$ prefactor is carried by the calling routine).
The result can equally be written as
\begin{equation}
\Delta M_{\mathrm{Hub},\gamma}
=
-\frac{\alpha}{2}
...
\end{equation}
%
where $\hat{\mathcal H}_\mathrm{KS}^\sigma$ is the KS Hamiltonian of the standard DFT, $\varepsilon_{v,{\bf k}}^\sigma$ is the $v$-th band energy at momentum ${\bf k}$ and spin $\sigma$, and $\hat{\mathcal V}_{U+V}^\sigma$ is the Hubbard potential that is derived from the Hubbard energy~\eqref{eq:Edftu} and which reads~\cite{Timrov:2021}:
%
\begin{eqnarray} 
